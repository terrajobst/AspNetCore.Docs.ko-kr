...
<xref:Microsoft.AspNetCore.Mvc.ApplicationModels.AttributeRouteModel.Order*>에 대한 <xref:Microsoft.AspNetCore.Mvc.ApplicationModels.AttributeRouteModel> 속성을 `1`로 설정합니다. 이렇게 하면 샘플 앱에서 다음과 같은 경로 일치 동작을 수행 합니다.

* `TheContactPage/{text?}`에 대 한 경로 템플릿은 항목의 뒷부분에 추가 됩니다. 연락처 페이지 경로는 기본 순서 `null` (`Order = 0`) 이므로 `{globalTemplate?}` 경로 템플릿 이전에 일치 합니다.
* `{aboutTemplate?}` 경로 템플릿은 항목의 뒷부분에 추가 됩니다. `{aboutTemplate?}` 템플릿이 `Order`의 `2`로 제공됩니다. `/About/RouteDataValue`에서 정보 페이지를 요청하는 경우 "RouteDataValue"는 `RouteData.Values["globalTemplate"]` 속성 설정으로 인해 `Order = 1`(`RouteData.Values["aboutTemplate"]`)이 아닌 `Order = 2`(`Order`)으로 로드됩니다.
* `{otherPagesTemplate?}` 경로 템플릿은 항목의 뒷부분에 추가 됩니다. `{otherPagesTemplate?}` 템플릿이 `Order`의 `2`로 제공됩니다. 경로 매개 변수를 사용 하 여 *Pages/otherpages* 폴더에 있는 페이지를 요청 하면 (예: `/OtherPages/Page1/RouteDataValue`)`Order = 2`속성을 설정 하 여 "RouteDataValue"가 `RouteData.Values["globalTemplate"]` (`Order = 1`)에 로드 되 고 `RouteData.Values["otherPagesTemplate"]` (`Order`)에 로드 되지 않습니다.
...
샘플 앱에서는 앱의 모든 페이지에 `AddHeaderAttribute` 헤더를 추가하는 `GlobalHeader` 클래스를 사용합니다.
...
샘플 앱은 `/TheContactPage`Contact.cshtml*의* 에 대한 경로를 만듭니다.

[!code-csharp[](razor-pages-conventions/samples/3.x/SampleApp/Startup.cs?name=snippet5)]

연락처 페이지도 기본 경로를 통해 `/Contact`에 도달할 수 있습니다.

연락처 페이지에 대한 샘플 앱의 사용자 지정 경로는 선택적 `text` 경로 세그먼트(`{text?}`)에 허용됩니다. 방문자가 해당 `@page` 경로에 있는 페이지에 액세스하는 경우 페이지에는 해당 `/Contact` 지시문에 있는 이 선택적 세그먼트가 포함됩니다.
...
이 샘플은 `AddFolderApplicationModelConvention`OtherPages`OtherPagesHeader` 폴더 내의 페이지에 *헤더를 추가하여 앱의*을 사용하는 방법을 보여줍니다.
...
샘플은 정보 페이지에 `AddPageApplicationModelConvention` 헤더를 추가하여 `AboutHeader`를 사용하는 방법을 보여줍니다.
...
샘플 앱은 앱의 페이지에 두 개의 값이 포함된 [ 헤더를 추가하여 ](xref:mvc/controllers/filters#ifilterfactory)필터 팩터리`FilterFactoryHeader`를 사용하는 예제를 제공합니다.
