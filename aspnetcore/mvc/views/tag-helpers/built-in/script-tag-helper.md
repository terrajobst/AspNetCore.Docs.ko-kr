...
다음 Razor 마크업은 대체(fallback)이 포함된 `script` 요소를 보여 줍니다.
...
CDN 스크립트 로드를 지연시키려면 `<script>` 요소의 [지연](https://developer.mozilla.org/docs/Web/HTML/Element/script) 특성을 사용하지 마세요. 스크립트 태그 도우미는 [asp-fallback-test](#asp-fallback-test) 식을 즉시 실행하는 JavaScript를 렌더링합니다. CDN 스크립트를 로드하는 작업이 지연되면 식이 실패합니다.
...
대체(fallback) 테스트에 사용할 기본 스크립트에 정의된 스크립트 메서드입니다. 자세한 내용은 <xref:Microsoft.AspNetCore.Mvc.TagHelpers.ScriptTagHelper.FallbackTestExpression>을 참조하세요.
