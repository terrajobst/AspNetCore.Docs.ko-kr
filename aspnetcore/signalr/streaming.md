...
[예제 코드 살펴보기 및 다운로드](https://github.com/aspnet/Docs/tree/live/aspnetcore/signalr/streaming/samples/) ([다운로드 방법](xref:index#how-to-download-a-sample))
...
하나 이상의 허용 하는 경우 자동으로 허브 메서드를 클라이언트와 서버 간 스트리밍 허브 메서드를 됩니다 <xref:System.Threading.Channels.ChannelReader`1>s입니다. 다음 샘플에서는 클라이언트에서 전송 하는 스트리밍 데이터를 읽는의 기본 사항을 보여 줍니다. 클라이언트에 쓸 때마다 합니다 <xref:System.Threading.Channels.ChannelWriter`1>는 데이터가 기록 되는 `ChannelReader` 허브 메서드에서 읽고 있는 서버에서.
...
.NET 클라이언트에서 클라이언트와 서버 간 스트리밍 허브 메서드를 호출 하려면 만들기를 `Channel` 전달 합니다 `ChannelReader` 인수로 `SendAsync`, `InvokeAsync`, 또는 `StreamAsChannelAsync`허브 메서드 호출에 따라 합니다.

데이터를 쓸 때마다는 `ChannelWriter`, 서버의 허브 메서드를 클라이언트에서 데이터를 사용 하 여 새 항목을 수신 합니다.

스트림의 종료 하려면 사용 하 여 채널 완료 `channel.Writer.Complete()`합니다.
