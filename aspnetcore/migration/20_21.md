...
* "Version" 특성에 대 한 패키지 참조를 제거 `Microsoft.AspNetCore.App`합니다. 사용 하는 프로젝트 `<Project Sdk="Microsoft.NET.Sdk.Web">` 버전을 설정할 필요가 없습니다. 버전이 대상 프레임 워크 사용 권한에 포함 된이 고 ASP.NET Core 2.1에서 작동 하는 방식에 가장 잘 맞도록 선택 합니다. 자세한 내용은 참조는 [공유 프레임 워크를 대상으로 하는 프로젝트에 대 한 규칙](#rules-for-projects-targeting-the-shared-framework) 섹션입니다.
...
* 삭제할 *Areas/Identity/IdentityHostingStartup.cs*합니다.
* 업데이트를  *\_LoginPartial.cshtml* 파일:
  * 이동할 *페이지 /\_LoginPartial.cshtml* 하려면 *페이지/Shared/\_LoginPartial.cshtml*합니다.
  * 추가 `asp-area="Identity"` 양식과 앵커 링크를 합니다.
  * 업데이트를 `<form />` 요소를 `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`입니다.
...
* *영역/Identity/페이지/\_ViewStart.cshtml*를 변경할 `Layout = "/Pages/_Layout.cshtml"` 에 `Layout = "/Pages/Shared/_Layout.cshtml"`입니다.
* 합니다  *\_Layout.cshtml* 파일에 다음과 같이 변경 합니다.

  * `<partial name="_CookieConsentPartial" />` 추가 됩니다. 자세한 내용은 [ASP.NET Core에서 GDPR 지원](xref:security/gdpr)을 참조하세요.
  * jQuery 3.3.1 2.2.0에서 변경 됩니다.

### <a name="validationscriptspartialcshtml"></a>\_ValidationScriptsPartial.cshtml

* *페이지 /\_ValidationScriptsPartial.cshtml* 이동할 *페이지/Shared/\_ValidationScriptsPartial.cshtml*합니다.
* *jquery.validate/1.14.0* 변경 내용 *jquery.validate/1.17.0*합니다.
