...
* @No__t_0에서 다음을 수행 합니다.

  * TFM를 `netcoreapp3.0` 업데이트 합니다.
  * @No__t_0 요소를 제거 합니다. 자세한 내용은이 문서의 [in-process 호스팅 모델](#in-process-hosting-model) 을 참조 하세요.
...
* @No__t_0 또는 `Microsoft.NET.Sdk.Razor` SDK를 대상으로 하는 프로젝트는 `Microsoft.AspNetCore.App`에 명시적 `FrameworkReference`를 추가 해야 합니다.
...
* [WebApi](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) &ndash; `System.Net.HttpClient`에 대 한 형식 지정 및 콘텐츠 협상 지원은 `ReadAsAsync`, `PostJsonAsync` 등의 api를 사용 하 여 `System.Net.HttpClient` 하는 데 유용한 확장성을 제공 합니다.
...
* MVC `Newtonsoft.Json`는 `Newtonsoft.Json`에서 MVC 사용에 대 한 &ndash; 지원 합니다. [NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson)의 일부입니다.
...
@No__t_0를 대상으로 하는 프로젝트는 이전에 [AspNetCore](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Analyzers/) 패키지의 일부로 제공 된 분석기를 암시적으로 참조 합니다. 이러한 기능을 사용 하도록 설정 하는 데 필요한 추가 참조는 없습니다.
...
* `GetTrailer` &ndash;은 요청 된 후행 헤더를 응답에서 가져옵니다. @No__t_1를 호출 하기 전에 `SupportsTrailers` 확인 하거나 요청이 후행 헤더를 지원 하지 않는 경우 <xref:System.NotSupportedException> 발생할 수 있습니다.
...
@No__t_1 옵션을 사용 하 여 `AllowSynchronousIO`를 사용 하는 것 외에도, 비동기 IO는 요청 별로 임시 완화 방법으로 재정의할 수 있습니다.
...
[Dispose](/dotnet/standard/garbage-collection/implementing-dispose)에서 동기 api를 호출 하는 <xref:System.IO.TextWriter> 구현 또는 다른 스트림에 문제가 있는 경우 대신 새 <xref:System.IO.Stream.DisposeAsync*> API를 호출 합니다.
...
ASP.NET Core 2.1에서는 *AspNetCore* 의 내용이 *AspNetCore*로 이동 되었습니다... n a m .dll. n a m .dll. @No__t_0 특성을 사용 하는 주요 업데이트가 아닙니다. 3\.0의 경우 빈 *AspNetCore* 어셈블리 및 NuGet 패키지는 제거 되었습니다 (예를 들어,
...
* @No__t_1를 호출 `Startup.ConfigureServices`를 업데이트 합니다.
...
@No__t_0에서 MVC 시나리오와 관련 된 세 가지 새로운 최상위 확장 메서드를 사용할 수 있습니다. 템플릿 `AddMvc` 대신 이러한 새 메서드를 사용 합니다. 그러나 `AddMvc`은 이전 릴리스와 동일 하 게 계속 작동 합니다.
 ...
 * @No__t_0를 사용 하 여 원하는 경로 이름을 전달 합니다.
...
* @No__t_0 또는 `UseSignalR`을 `UseEndpoints` 바꿉니다.
...
* @No__t_0을 `IWebHostEnvironment`로 바꾸고 <xref:Microsoft.Extensions.Hosting?displayProperty=fullName> 네임 스페이스에 대 한 `using` 문을 추가 합니다.
* @No__t_0를 <xref:Microsoft.Extensions.Hosting.IHostApplicationLifetime> (<xref:Microsoft.Extensions.Hosting?displayProperty=fullName> 네임 스페이스)로 바꿉니다.
* @No__t_0를 <xref:Microsoft.Extensions.Hosting.Environments> (<xref:Microsoft.Extensions.Hosting?displayProperty=fullName> 네임 스페이스)로 바꿉니다.
...
* @No__t_0 클래스는 `[DisableCors]` 특성을 사용 하 여 CORS를 사용 하지 않도록 설정 합니다.
...
이전 버전의 ASP.NET Core에서는 `[Authorize]` 특성을 통해 권한 부여 지원이 제공 되었습니다. 권한 부여 미들웨어를 사용할 수 없습니다. ASP.NET Core 3.0에서는 권한 부여 미들웨어가 필요 합니다. @No__t_1 직후에 `UseAuthorization` (ASP.NET Core 권한 부여 미들웨어)를 배치 하는 것이 좋습니다. 권한 부여 미들웨어는 재정의 될 수 있는 기본 정책으로 구성 될 수도 있습니다.
...
@No__t_0는 처음에 인증을 요구 하도록 구성 되므로 추가 구성이 필요 하지 않습니다. 다음 예제에서 MVC 끝점은 `RequireAuthorization`으로 표시 되므로 모든 요청을 `DefaultPolicy`을 기반으로 하 여 권한을 부여 해야 합니다. 그러나 `HomeController` `[AllowAnonymous]`으로 인해 사용자가 앱에 로그인 하지 않아도 액세스할 수 있습니다.
...
또는 `FallbackPolicy`를 구성 하 여 `[Authorize]` 하거나 `RequireAuthorization` 하지 않고 권한 부여를 요구 하도록 모든 끝점을 구성할 수 있습니다. @No__t_0 `DefaultPolicy`와 다릅니다. @No__t_0은 `[Authorize]` 또는 `RequireAuthorization`에서 트리거되고 다른 정책이 설정 되지 않은 경우에도 `FallbackPolicy` 트리거됩니다. `FallbackPolicy`은 처음에 권한 부여 없이 요청을 허용 하도록 구성 됩니다.
...
@No__t_0에 각 허브를 매핑합니다. 이전 버전과 마찬가지로 각 허브는 명시적으로 나열 됩니다.
...
라우팅은 이제 MVC 이상에 대 한 지원을 포함 하므로 이러한 메서드가 수행 하는 작업을 명확 하 게 나타내도록 용어가 변경 되었습니다. @No__t_0 / `MapAreaControllerRoute` / `MapDefaultControllerRoute` 등의 기존 경로는 추가 된 순서 대로 적용 됩니다. 먼저 특정 경로 (예: 영역에 대 한 경로)를 추가 합니다.
...
* @No__t_0 접미사를 지정 하는 데 필요한 링크가 생성 되었습니다. 예를 들면,
...
이 변경 내용은 [[ActionName]](/dotnet/api/microsoft.aspnetcore.mvc.actionnameattribute) 특성을 사용 하 여 지정 된 이름에는 영향을 주지 않습니다. @No__t_0에서 다음 코드를 사용 하 여 기본 동작을 사용 하지 않도록 설정할 수 있습니다.
...
끝점 라우팅과 함께 상태 검사를 사용 하려면 `MapHealthChecks`을 추가 합니다. @No__t_0 메서드는 `UseHealthChecks`와 유사한 인수를 허용 합니다. @No__t_1에 대해 `MapHealthChecks`를 사용 하는 이점은 권한 부여를 적용 하 고 일치 정책에 대 한 보다 세분화 된 제어를 제공 하는 기능입니다.
...
<xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder>은 3.0에 유지 되며 앞의 코드 샘플에 표시 된 `webBuilder`의 유형입니다. <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder>은 이후 릴리스에서 더 이상 사용 되지 않으며 `HostBuilder`로 바뀝니다.

@No__t_0에서 `HostBuilder`로의 가장 중요 한 변경 내용은 [DI (종속성 주입)](xref:fundamentals/dependency-injection)에 있습니다. @No__t_0를 사용 하는 경우 `Startup`의 생성자에만 다음을 삽입할 수 있습니다.
...
* 부트스트랩 4는 기본 UI 프레임 워크입니다. @No__t_0 프로젝트 속성을 설정 하 여 기본값을 변경 합니다. 자세한 내용은 [이 GitHub 공지](https://github.com/aspnet/Announcements/issues/380)를 참조 하세요.
...
@No__t_0에서 `AddJsonProtocol`를 호출 하 여 serializer 옵션을 설정 합니다.
...
@No__t_1에서 지원 되지 않는 `Newtonsoft.Json` 기능을 사용 하는 경우 `Newtonsoft.Json`로 다시 전환할 수 있습니다.
...
1. @No__t_1를 호출 하 `Startup.ConfigureServices`를 업데이트 합니다.
...
@No__t_0 샘플에서는 개발 환경에서 조건부로 런타임 컴파일을 사용 하도록 설정 하는 예제를 보여 줍니다.
...
@No__t_0를 사용 하 여 ASP.NET Core 3.0 특정 Api를 사용 하도록 설정 합니다.
...
@No__t_0를 [일반 호스트](xref:fundamentals/host/web-host)와 직접 사용 하는 앱의 경우 <xref:Microsoft.Extensions.Hosting.GenericHostWebHostBuilderExtensions.ConfigureWebHost*>의 <xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder>에 `TestServer`를 만듭니다.
