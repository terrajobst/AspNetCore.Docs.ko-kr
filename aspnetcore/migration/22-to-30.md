...
앱에서 docker를 사용 하는 경우 ASP.NET Core 3.0를 포함 하는 기본 이미지를 사용 합니다. 예를 들어 `docker pull mcr.microsoft.com/dotnet/core/aspnet:3.0`과 같은 형식입니다.
...
* MVC `Newtonsoft.Json` 지원 &ndash; @no__t에서 MVC 사용에 대 한 지원 현재 [AspNetCore](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson)의 일부입니다.
...
@No__t-0을 대상으로 하는 프로젝트는 이전에 [AspNetCore](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Analyzers/) 패키지의 일부로 제공 된 분석기를 암시적으로 참조 합니다. 이러한 기능을 사용 하도록 설정 하는 데 필요한 추가 참조는 없습니다.
...
* `Microsoft.AspNetCore.Server.Kestrel.Transport.Abstractions.Internal.SchedulingMode`이 <xref:Microsoft.AspNetCore.Server.Kestrel.KestrelServerOptions>에서 제거 되었습니다.
...
@No__t-0 구현이 나 [Dispose](/dotnet/standard/garbage-collection/implementing-dispose)에서 동기 api를 호출 하는 기타 스트림에 문제가 있는 경우 대신 새 <xref:System.IO.Stream.DisposeAsync*> API를 호출 합니다.
...
@No__t-0에서 MVC 시나리오와 관련 된 세 가지 새로운 최상위 확장 메서드를 사용할 수 있습니다. 템플릿 `AddMvc` 대신 이러한 새 메서드를 사용 합니다. 그러나 `AddMvc`은 이전 릴리스와 동일 하 게 계속 작동 합니다.
...
자세한 내용은 <xref:host-and-deploy/health-checks>을 참조하십시오.
...
ASP.NET Core 3.0에서 ASP.NET Core MVC는 컨트롤러 작업 이름에서 `Async` 접미사를 제거 합니다. 라우팅 및 링크 생성은 모두이 새 기본값의 영향을 받습니다. 예를 들면,
...
* @No__t-0 접미사를 지정 하는 링크 생성이 필요 합니다. 예를 들면,
...
* 링크 생성에 `Async` 접미사를 지정할 필요가 없습니다. 예를 들면,
...
자세한 내용은 [이 GitHub 문제](https://github.com/aspnet/EntityFrameworkCore/issues/17788)를 참조하세요.
