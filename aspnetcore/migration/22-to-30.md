...
* AspNetCore에 패키지 참조를 추가 합니다. [NewtonsoftJson](https://nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
* `UseCors` 앱에서 또는 `[Authorize]`와 `AuthorizePage` 같은 인증/권한 부여 기능을 사용 하는 경우 호출 `UseAuthentication` 을 `UseAuthorization` 및 **뒤** `UseRouting` (CORS 미들웨어를 사용 하는 경우)에를 추가 합니다.
...
응용 프로그램에서를 `AuthorizeFilter` MVC의 전역 필터로 사용 하는 경우 `UseAuthorization` 미들웨어에 정책을 제공 하도록 코드를 리팩터링 하는 것이 좋습니다.

다음 예제에서는가 호출 될 때 `UseAuthorization` 모든 요청에 적용 되는 사용자 지정 정책을 사용 하 여 사용자가 앱에 로그인 하지 않아도 액세스를 허용 합니다. `HomeController`
...
다음 예제 `UseAuthorization` 에서는 기본 정책 없이 권한 부여를 처리 `/healthz` 하지만 상태 검사 끝점을 사용 하려면 `admin` 사용자가 역할에 있어야 합니다. 
