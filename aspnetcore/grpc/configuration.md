...
| `LoggerFactory` | `null` | 클라이언트에서 grpc 호출에 대 한 정보를 기록 하 는데사용하는입니다.`LoggerFactory` 인스턴스 `LoggerFactory` 는 종속성 주입에서 확인 하거나를 사용 하 여 `LoggerFactory.Create`만들 수 있습니다. 로깅 구성 예제는를 참조 <xref:grpc/diagnostics#grpc-client-logging>하십시오. |
