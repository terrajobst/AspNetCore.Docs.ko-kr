...
1. Entity Framework에서 새 모델 클래스 스키마에 따라 데이터베이스를 자동으로 삭제하고 다시 만들도록 합니다. 이 방법은 테스트 데이터베이스에서 활발한 개발을 수행할 때 개발 주기의 초기 단계에서 매우 편리합니다. 신속하게 모델 및 데이터베이스 스키마를 함께 발전시킬 수 있습니다. 그러나 데이터베이스에서 기존 데이터가 손실된다는 단점이 있습니다. 따라서 프로덕션 데이터베이스에서는 이 방법을 사용하지 마세요! 이니셜라이저를 사용하여 자동으로 데이터베이스를 테스트 데이터로 시드하는 것은 종종 응용 프로그램을 개발하기 위한 효율적인 방법입니다. 이는 초기 개발에 그리고 SQLite 사용 시 좋은 방법입니다.
