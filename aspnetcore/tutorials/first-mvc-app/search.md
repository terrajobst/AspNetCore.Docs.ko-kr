...
`Index`Controllers/MoviesController.cs*에 있는*  메서드를 다음 코드로 수정합니다.
...
`Index`라는 매개 변수를 포함하도록 `id` 메서드의 서명을 변경하면 해당 `id` 매개 변수는 `{id}`Startup.cs*에서 설정된 기본 경로의 선택적*  자리 표시자와 일치합니다.
...
`Index` 매개 변수를 사용하는 수정된 `id` 메서드는 다음과 같습니다.
...
그러나 사용자가 영화를 검색하려고 할 때마다 URL을 수정하리라고 기대하지는 않을 것입니다. 따라서 이제 영화를 필터링하는 데 도움이 되는 UI 요소를 추가할 것입니다. 경로 바인딩 `Index` 매개 변수의 전달 방법을 테스트하기 위해 `ID` 메서드의 서명을 변경했다면, `searchString`이라는 매개 변수를 사용하도록 메서드를 다시 변경합니다.
...
예상할 수 있는 것처럼 `[HttpPost]` 메서드의 `Index` 오버로드는 없습니다. 이 메서드는 앱의 상태를 변경하지 않고 데이터를 필터링하기만 하기 때문에 필요하지 않습니다.
...
검색 매개 변수가 URL이 아닌 요청 본문에 존재하기 때문에 책갈피에 해당 검색 정보를 캡처하거나 다른 사용자와 공유할 수 없습니다. `HTTP GET`Views/Movies/Index.cshtml*파일에서* 요청을 지정하여 이 문제를 해결합니다.

[!code-html[](~/tutorials/first-mvc-app/start-mvc/sample/MvcMovie22/Views/Movies/IndexGet.cshtml?highlight=12&range=1-23)]

이제 검색을 제출하면 URL에 검색 쿼리 문자열이 포함됩니다. `HttpGet Index` 메서드가 존재하더라도 검색은 `HttpPost Index` 작업 메서드로 이동합니다.
...
`Index`에서 `MoviesController.cs` 메서드를 다음 코드로 바꿉니다.
...
`Index.cshtml`Views/Movies/*에 있는* 을 다음과 같이 업데이트합니다.
