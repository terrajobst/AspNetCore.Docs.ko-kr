...
1. 대상 프레임워크 드롭다운에서 *.NET Core*를 선택하고, 프레임워크 선택기 드롭다운에서 *ASP.NET Core 3.0*을 선택합니다. **빈** 템플릿을 선택하고 **만들기**를 선택합니다.
...
1. `Startup.Configure` 메서드 끝에서 */hub* 경로를 `ChatHub` 허브에 매핑합니다. *Hello World!* 를 표시하는 코드를 다음 줄로 바꿉니다. 
