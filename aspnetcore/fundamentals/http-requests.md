...
* 문자열을 키로 사용할 필요 없이 명명된 클라이언트와 동일한 기능을 제공합니다.
* 클라이언트를 사용할 때 IntelliSense 및 컴파일러 지원을 제공합니다.
* 특정 `HttpClient`를 구성하고 상호 작용하기 위한 단일 위치를 제공합니다. 예를 들어 단일 형식화된 클라이언트는 단일 백 엔드 엔드포인트에 사용될 수 있으며 해당 엔드포인트를 다루는 모든 논리를 캡슐화할 수 있습니다.
* DI로 작업하고 앱의 필요한 곳에서 주입할 수 있습니다.
...
콘솔 앱에서는 프로젝트에 다음 패키지 참조를 추가합니다.
...
* <xref:System.Net.Http.IHttpClientFactory>가 [제네릭 호스트의](xref:fundamentals/host/generic-host) 서비스 컨테이너에 등록됩니다.
* `MyService`는 서비스로부터 `HttpClient`를 만드는 데 사용되는 클라이언트 팩터리 인스턴스를 만듭니다. `HttpClient`는 웹 페이지를 검색하는 데 사용됩니다.
* `Main`은 서비스의 `GetPage` 메서드를 실행하고 웹 페이지 콘텐츠의 처음 500자를 콘솔에 쓰기 위한 영역을 만듭니다.
