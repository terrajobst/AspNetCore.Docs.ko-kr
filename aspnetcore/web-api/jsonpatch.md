...
* `AddNewtonsoftJson`에 대한 호출을 포함하도록 프로젝트의 `Startup.ConfigureServices` 메서드를 수정합니다.
...
`AddNewtonsoftJson`은 **모든** JSON 콘텐츠의 형식을 지정하는 데 사용되는 `System.Text.Json` 기반 입력 및 출력 포맷터를 대체합니다. `Newtonsoft.Json`를 사용하여 `JsonPatch` 지원을 추가하지만 다른 포맷터를 변경하지 않으려면 프로젝트의 `Startup.ConfigureServices`를 다음과 같이 업데이트합니다.
