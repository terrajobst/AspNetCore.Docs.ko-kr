...
Blazor 서버 쪽은 상태 저장 앱 프레임 워크입니다. 대부분의 경우 앱은 서버에 지속적으로 연결을 유지 합니다. 사용자의 상태는 *회로*의 서버 메모리에 보관 됩니다. 
...
> [!NOTE]
> 이 문서에서는 Blazor 서버 쪽 앱의 상태 지 속성에 대해 다룹니다. Blazor 클라이언트 쪽 앱은 [브라우저에서 클라이언트 쪽 상태 지 속성](#client-side-in-the-browser) 을 활용할 수 있지만이 문서의 범위를 벗어나는 사용자 지정 솔루션 또는 타사 패키지를 요구 합니다.
...
Blazor 서버 쪽 앱에서 상태를 유지 하기 위한 세 가지 공통 위치가 있습니다. 각 방법은 서로 다른 시나리오에 가장 적합 하며, 다음과 같은 다양 한 주의 사항이 있습니다.
...
> [!NOTE]
> 이 단원의 "클라이언트 쪽"은 [Blazor 클라이언트 쪽 호스팅 모델이](xref:blazor/hosting-models#client-side)아닌 브라우저의 클라이언트 쪽 시나리오를 나타냅니다. `localStorage`및 `sessionStorage` 는 Blazor 클라이언트 쪽 앱에서 사용할 수 있지만 사용자 지정 코드를 작성 하거나 타사 패키지를 사용 하는 경우에만 사용할 수 있습니다.
...
* Blazor 서버 쪽 앱의 경우 몇 킬로바이트의 데이터를 저장 하는 것이 합리적입니다. 네트워크를 통해 데이터를 로드 하 고 저장 하기 때문에 몇 킬로바이트 이상에서 성능 영향을 고려해 야 합니다.
...
1. Blazor 서버 쪽 앱 프로젝트에서 [AspNetCore ProtectedBrowserStorage](https://www.nuget.org/packages/Microsoft.AspNetCore.ProtectedBrowserStorage)에 패키지 참조를 추가 합니다.
