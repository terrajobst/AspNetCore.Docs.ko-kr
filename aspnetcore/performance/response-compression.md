...
일반적으로 기본적으로 압축 되지 않은 모든 응답은 응답 압축의 이점을 누릴 수 있습니다. 기본적으로 압축 되지 않은 응답은 일반적으로 다음을 포함 합니다. CSS, JavaScript, HTML, XML 및 JSON이 있습니다. PNG 파일과 같이 기본적으로 압축 된 자산은 압축 해서는 안 됩니다. 고유 하 게 압축 된 응답을 추가로 압축 하려는 경우 크기 및 전송 시간의 작은 추가 감소는 압축을 처리 하는 데 걸린 시간을 overshadowed 수 있습니다. 파일의 내용과 압축 효율성에 따라 약 150-1000 바이트 보다 작은 파일은 압축 하지 않습니다. 작은 파일을 압축 하는 오버 헤드로 인해 압축 되지 않은 파일 보다 큰 압축 된 파일이 생성 될 수 있습니다.
...
미들웨어는 압축 체계의 우선 순위를 지정 하기 위해 클라이언트에서 `q`보낼 때 품질 값 (qvalue,) 가중치에 대응 시킬 수 있습니다. 자세한 내용은 RFC 7231을 [참조 하세요. Accept-Encoding](https://tools.ietf.org/html/rfc7231#section-5.3.4).
...
| `Content-Length`   | 압축이 발생 `Content-Length` 하면 응답이 압축 될 때 본문 내용이 변경 되므로 헤더가 제거 됩니다. |
| `Content-MD5`      | 압축이 발생 `Content-MD5` 하면 본문 내용이 변경 되 고 해시가 더 이상 유효 하지 않기 때문에 헤더가 제거 됩니다. |
...
* `app.UseResponseCompression`이전 `app.UseMvc`에를 호출 해야 합니다.
* [Fiddler](https://www.telerik.com/fiddler), [Firebug](https://getfirebug.com/)또는 `Accept-Encoding` [postman](https://www.getpostman.com/) 과 같은 도구를 사용 하 여 요청 헤더를 설정 하 고 응답 헤더, 크기 및 본문을 연구 합니다.
...
을 사용 하 여 [Brotli 압축 된 데이터 형식의](https://tools.ietf.org/html/rfc7932)응답을 압축 합니다. <xref:Microsoft.AspNetCore.ResponseCompression.BrotliCompressionProvider>

에 명시적으로 추가 <xref:Microsoft.AspNetCore.ResponseCompression.CompressionProviderCollection>된 압축 공급자가 없는 경우:
...
Nginx에서 요청을 프록시 하는 경우 `Accept-Encoding` 헤더가 제거 됩니다. `Accept-Encoding` 헤더를 제거 하면 미들웨어가 응답을 압축 하지 않습니다. 자세한 내용은 [NGINX: 압축 및](https://www.nginx.com/resources/admin-guide/compression-and-decompression/)압축 해제 이 문제는 [Nginx에 대 한 통과 압축 (aspnet/basicmiddleware \#123)](https://github.com/aspnet/BasicMiddleware/issues/123)을 통해 추적 됩니다.
...
* 헤더 `Accept-Encoding` 는 설정 된 사용자 지정 압축 공급자 `br`와 일치 `*`하는, `gzip`, 또는 사용자 지정 인코딩의 값을 포함 합니다. 값이 `identity` 이거나, 값이 0 인 품질 값 (qvalue, `q`)이 설정 되어 있지 않아야 합니다.
...
* [Mozilla 개발자 네트워크: Accept-Encoding](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Encoding)
* [RFC 7231 섹션 3.1.2.1: 콘텐츠 Codings](https://tools.ietf.org/html/rfc7231#section-3.1.2.1)
