...
* 페이지로리디렉션되`/Identity/Account/Manage/PersonalData` 는 `Hello YourEmail@provider.com!` 링크를 선택 합니다.
...
`SendGridUser` [암호 관리자 도구](xref:security/app-secrets)를 사용 하 여 및 `SendGridKey` 를 설정 합니다. 예:
...
전자 메일 링크를 클릭 하 여 로컬 및 소셜 계정을 결합할 수 있습니다. 다음 시퀀스에서는 ""이RickAndMSFT@gmail.com(가) 로컬 로그인으로 먼저 생성 되지만 계정을 소셜 로그인으로 먼저 만든 다음 로컬 로그인을 추가할 수 있습니다.
...
앱을 실행 하 고, **등록** 링크를 선택 하 고, 사용자를 등록 합니다. 이때 전자 메일에 대 한 유일한 유효성 검사는 [[EmailAddress]](/dotnet/api/system.componentmodel.dataannotations.emailaddressattribute) 특성을 사용 하는 것입니다. 등록을 제출 하면 앱에 로그인 됩니다. 자습서의 뒷부분에서 새로운 사용자가 전자 메일의 유효성을 검사할 때까지 로그인 할 수 없도록 코드를 업데이트 합니다.

[!INCLUDE[](~/includes/view-identity-db.md)]

테이블의 `EmailConfirmed` `False`필드는입니다.
...
새 사용자 등록의 전자 메일을 확인 하는 것이 가장 좋습니다. 전자 메일 확인을 사용 하면 다른 사용자를 가장 하지 않는 것을 확인할 수 있습니다. 즉, 다른 사용자의 전자 메일을 사용 하 여 등록 하지 않았습니다. 토론 포럼이 있고 ""을 ""yli@example.comnolivetto@contoso.com로 등록 하지 못하도록 하는 경우를 가정해 보겠습니다. 전자 메일 확인이 없으면nolivetto@contoso.com""이 (가) 앱에서 원치 않는 전자 메일을 받을 수 있습니다. "ylo@example.com"로 실수로 등록 한 사용자가 "yli"의 맞춤법 오류를 발견 했다고 가정 합니다. 앱에 올바른 전자 메일이 없기 때문에 암호 복구를 사용할 수 없습니다. 전자 메일 확인은 봇에서 제한 된 보호 기능을 제공 합니다. 전자 메일 확인은 전자 메일 계정이 많은 악의적인 사용자 로부터 보호를 제공 하지 않습니다.
...
템플릿에는 계정 확인 및 암호 복구를 위한 코드가 있습니다. `OnPostAsync` *영역/i d/페이지/계정/등록. cshtml*에서 메서드를 찾습니다.
