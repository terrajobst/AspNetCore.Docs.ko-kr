...
* Using 지시문 목록에 `using Microsoft.AspNetCore.Http`를 추가 합니다.
* `Startup.ConfigureServices` 및 [UseCookiePolicy](/dotnet/api/microsoft.aspnetcore.builder.cookiepolicyappbuilderextensions.usecookiepolicy) 에 [CookiePolicyOptions](/dotnet/api/microsoft.aspnetcore.builder.cookiepolicyoptions) 를 `Startup.Configure`에 추가 합니다.
...
* 프로젝트에 *\_CookieConsentPartial* 파일을 추가 합니다.
...
* 프로젝트 템플릿에는 개인 정보 및 쿠키 사용 정책으로 바꿀 수 있는 확장 지점과 스텁 태그가 포함 됩니다.
* 쿠키 동의 기능을 사용 하면 개인 정보를 저장 하기 위해 사용자의 동의를 요청 하 고 추적할 수 있습니다. 사용자가 데이터 수집에 동의한 하지 않고 앱이 `true`로 [설정 되어 있는 경우에는](/dotnet/api/microsoft.aspnetcore.builder.cookiepolicyoptions.checkconsentneeded) 중요 하지 않은 쿠키가 브라우저로 전송 되지 않습니다.
...
* [CookiePolicyOptions](/dotnet/api/microsoft.aspnetcore.builder.cookiepolicyoptions) 및 [UseCookiePolicy](/dotnet/api/microsoft.aspnetcore.builder.cookiepolicyappbuilderextensions.usecookiepolicy) 는 `Startup` 클래스에서 설정 됩니다.
* *\_CookieConsentPartial* [부분 뷰입니다](xref:mvc/views/tag-helpers/builtin-th/partial-tag-helper). **수락** 단추가이 파일에 포함 됩니다. 사용자가 **동의** 단추를 클릭 하면 쿠키를 저장 하는 데 대 한 동의가 제공 됩니다.
...
[CookiePolicyOptions](/dotnet/api/microsoft.aspnetcore.builder.cookiepolicyoptions) 는 `Startup.ConfigureServices`에서 초기화 됩니다.

[!code-csharp[Main](gdpr/sample/Startup.cs?name=snippet1&highlight=14-20)]

[UseCookiePolicy](/dotnet/api/microsoft.aspnetcore.builder.cookiepolicyappbuilderextensions.usecookiepolicy) 는 `Startup.Configure`에서 호출 됩니다.
...
* 사용자에 대 한 추적 상태를 가져옵니다. 앱이 동의를 요구 하도록 구성 된 경우 사용자는 쿠키를 추적할 수 있도록 먼저 동의 해야 합니다. 동의가 필요한 경우 쿠키 승인 패널은 *\_Layout. cshtml* 파일에 의해 생성 된 탐색 모음 위에 고정 됩니다.
* 개인 정보 및 쿠키 사용 정책을 요약 하는 HTML `<p>` 요소를 제공 합니다.
...
[TempData 공급자](xref:fundamentals/app-state#tempdata) 쿠키는 필수적이 지 않습니다. 추적을 사용 하지 않도록 설정 하면 TempData 공급자가 작동 하지 않습니다. 추적이 사용 하지 않도록 설정 된 경우 TempData 공급자를 사용 하도록 설정 하려면 `Startup.ConfigureServices`에서 TempData 쿠키를 필수로 표시 합니다.
...
* Id 데이터베이스 테이블에 저장 된 사용자에 대 한 저장 된 토큰은 [외래 키](https://github.com/aspnet/Identity/blob/release/2.1/src/EF/IdentityUserContext.cs#L152)로 인해 cascade delete 동작을 통해 사용자를 삭제할 때 삭제 됩니다 `AspNetUserTokens`.
