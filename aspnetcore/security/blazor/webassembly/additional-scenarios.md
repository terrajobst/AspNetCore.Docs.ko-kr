...
SPA (단일 페이지 응용 프로그램)가 OIDC (Open ID Connect)를 사용 하 여 사용자를 인증 하는 경우 인증 상태는 SPA 내에서 로컬로 유지 관리 되 고, 사용자가 해당 사용자의 로그인을 제공 하는 사용자의 결과로 설정 된 세션 쿠키 형식으로 IP 자격 증명.
