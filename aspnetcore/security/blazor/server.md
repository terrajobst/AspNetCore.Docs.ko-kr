...
* 모든 호출에는 장애 조치 (failover) 후 구성 가능한 시간 제한이 있으므로 호출자에 게 <xref:System.OperationCanceledException>을 반환 합니다.
  * 호출 (`CircuitOptions.JSInteropDefaultCallTimeout`)의 기본 시간 제한은 1 분입니다. 이 제한을 구성 하려면 <xref:blazor/call-javascript-from-dotnet#harden-js-interop-calls>를 참조 하세요.
...
* `CircuitOptions.DetailedErrors`입니다.
